...
def detrend_signals(signals):
    ...
    detrend = [signals[k] - trend[k]
               for k in range(len(signals))]  # Removes trend from signal
...
def RR_interval(peaklist, fs):
    ...
    while (cnt < (len(peaklist)-1)):
        ...
        # Convert sample distances to ms distances (fs로 나눠서 1초단위로 거리표현 -> 1ms단위로 change)
...
def calc_heartrate(RR_list):
    ...
    for val in RR_list:
        if val > 400 and val < 1500:
            # 60000 ms /1 minute. time per beat(한번 beat하는데 걸리는 시간)
...
def FFT(block, fs):
    # divide by len(block) to normalize
    fourierTransform = np.fft.fft(block)/len(block)
    # single side frequency / symmetric
...
def compute_and_reconstruction_dft(data, fs, sec, overlap, cutoff):
    ...
    for i in range(0, len(data), fs*sec-overlap):
        ...
        # 2% overlapping
        if i == 0:
            concatenated_sig = np.hstack(
                [concatenated_sig, filtered_sig[:fs*sec - overlap//2]])
        elif i == len(data)-1:
            concatenated_sig = np.hstack(
                concatenated_sig, filtered_sig[overlap//2:])
        else:
            concatenated_sig = np.hstack(
                [concatenated_sig, filtered_sig[overlap//2:fs*sec - overlap//2]])
...
def statistic_threshold(clean_signal, fs, ths):
    ...
    skews_ths = [np.mean(skews) - ths[2], np.mean(skews) +
                 ths[3]]  # -0.3 and 0.8   -0.5 , 0.9
...
def eliminate_noise_in_time(data, fs, ths, cycle=1):
    ...
    stds_ = [np.mean(stds[i:i+cycle])
             for i in range(0, len(stds)-cycle+1, cycle)]
    kurtosiss_ = [np.mean(kurtosiss[i:i+cycle])
                  for i in range(0, len(kurtosiss)-cycle+1, cycle)]
    skews_ = [np.mean(skews[i:i+cycle])
              for i in range(0, len(skews)-cycle+1, cycle)]
    ...
    eli_skew = [skews_.index(x)
                for x in skews_ if x > ths[2][0] and x < ths[2][1]]
    ...
    for x in new_list:
        index.extend(
            [x for x in range(valley[x*cycle][0], valley[x*cycle+cycle-1][1], 1)])
...
def auto_correlation(filtered_3sec):
    ...
    a = plt.acorr(filtered_3sec, usevlines=True, normed=True,
                  maxlags=(len(filtered_3sec)-1), lw=2)
    valley_indexes = signal.argrelextrema(a[1], np.less)  # find valley index
    plt.scatter(x=valley_indexes[0]-(len(filtered_3sec)-1),
                y=a[1][valley_indexes[0]], color='red', s=20)
